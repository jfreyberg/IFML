...
def can_run_or_hide(self, state, start, action='other'):
    ...
    map = game_state['field'].copy()
    obstacles = [(x,y) for ((x,y),t) in game_state['bombs']]
    obstacles.extend([(x,y) for (_, _, _, (x,y)) in game_state['others']])
    for o in obstacles:
        map[o] = -2
...
def funcs(self, state, action):
    ...
    _, _, bombs_left, (x, y) = game_state['self']                # our coordinates
...
def act(self, game_state: dict):
    ...
    arena = game_state['field']                                # map
    _, _, bombs_left, (x, y) = game_state['self']                # our coordinates
    bomb_xys = [(x,y) for ((x,y),t) in game_state['bombs']]      # bomb coordinates
    others = [(x,y) for (_, _, _, (x,y)) in game_state['others']]   # other agents' coordinates
    ...
    self.state = [(game_state).copy(), list(self.coordinate_history), bomb_map.copy()] 
...
def reward_update(self):
    ...
    for event in self.events:
        if event in [0,1,2,3]:          # MOVED left, right, up or down
            ...
            x,y = self.me
            for p in [(min(x+2,16),y), (max(x-2,0),y),(x,min(y+2,16)),(x,max(y-2,0)),(x+1,y+1),(x+1,y-1),(x-1,y+1),(x-1,y-1)]:
                if p in [(x,y) for (_, _, _, (x,y)) in game_state['others']]:
